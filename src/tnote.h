...
enum Eacidentals { e_Natural = 0, e_Sharp=1, e_DoubleSharp=2,
        ...
        /** number variable is a number in "diatonal notation" (see constructor).*/
char number() {return m_number; }
        /** Octave number is @p 0 for "small octave",  @p -1 for "Great" @p 1 for "one-line". */
char octave() {return m_octave; }
...
char accidental() {return m_accid; }

static const QString Letters[7];
static const QString Solmization[7];
static const QString signsAcid[5];
...
Tnote (char _noteNr, char _octave, char _accidental = 0);
...
void setNumber(char nr) {m_number = nr; }
...
Tnote showWithDoubleFlat();
          ...
          * @param otherNote
          * @param ignoreOctave .If (TRUE) the octave values are ignored,
          * and method compares only number of note and acidental.
          * @return 1 for TRUE or 0 if notes are different
          */
bool compareNotes(Tnote otherNote, bool ignoreOctave = false);
     /**
      * Return std::string with note in format determines by notation
      * @param showOctave
      * @return DEPRECATED
      */
//QString printNote ( bool showOctave = 1);

        QString getName (EnameStyle nameStyle = e_norsk_Hb, bool showOctave = 1);
        QString getName (Tnote eNote, EnameStyle nameStyle = e_norsk_Hb, bool showOctave = 1);
...
private:
    char m_number, m_accid, m_octave;
