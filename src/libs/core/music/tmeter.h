...
public:
    ...
    enum EbeatUnit { BeatQuarter = 0, BeatEighth = 1, BeatQuarterDot = 2, BeatHalf = 3 };
     ...
     */
    static int quarterTempo(int beatTempo, Tmeter::EbeatUnit beatUnit) { return qRound(static_cast<qreal>(beatTempo) / beatTempoFactor(beatUnit)); }

    /**
     * Tempo of quarter notes (per minute), independent on beat unit
     */
    static int quarterTempo(int beatTempo, int beatUnit) { return Tmeter::quarterTempo(beatTempo, static_cast<EbeatUnit>(beatUnit)); }
