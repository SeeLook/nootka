...
public:
     ...
     */
    enum Erhythm : quint8 { NoRhythm = 0, Whole = 1, Half = 2, Quarter = 3, Eighth = 4, Sixteenth = 5 };
    Q_ENUM(Erhythm)

    /**
     * Additional note preferences
     */
    enum Eprefs : quint8 { e_rest = 1, e_dot = 2, e_triplet = 4, e_stemDown = 8 };

    /**
     * It covers 16&32 bits of @p m_prefs value. It is mapped into @enum Ebeam then
     */
    enum Ebeam : quint8 { e_noBeam = 0, e_beamStart = 16, e_beamCont = 32, e_beamEnd = 48 };
    Q_ENUM(Ebeam)

    enum Etie : quint8 { e_noTie = 0, e_tieStart = 64, e_tieCont = 128, e_tieEnd = 192 };
    Q_ENUM(Etie)

    /**
     * DEFAULT CONSTRUCTOR: by default it is quarter (without dot)
     */
    Trhythm(Erhythm nVal = Quarter, bool rest = false, bool dot = false, bool triplet = false) { setRhythm(nVal, rest, dot, triplet); }
     ...
     */
    int duration() const { return durArray[m_r][hasDot() ? 1 : (isTriplet() ? 2 : 0)]; }
