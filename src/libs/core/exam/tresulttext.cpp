...
QString wasAnswerOKtext(TQAunit *answer, int attempt)
{
    ...
    if (curQ.isCorrect()) {
        txt += QGuiApplication::translate(
            "AnswerText",
            "Good answer!",
            "or 'Good!' or 'Correct!' would be somewhat more specific than merely 'It was good!' (previous version) 'It' in this case certainly does refer to "
            "a specific thing, which is in this case the answer, but it might be momentarily confused with some other specific thing, such as a shoe or a "
            "crocodile, or the wind on one's back. I know that's probably confusing, but the implied subject of 'Correct! is in a certain sense much more "
            "specific than a mere 'It' and is more certain to refer to the answer.");
    } else if (curQ.wrongNote() || curQ.wrongPos() || curQ.veryPoor())
        ...
        txt +=
            QGuiApplication::translate(
                "AnswerText",
                "Not bad, but:",
                "'Not so bad, but:' is perfectly clear, but a little less common in US English. To be a bit shorter, it might just as well be, 'Not bad, but:'")
            + QLatin1String("<br>");
