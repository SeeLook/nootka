...
qreal TscoreObject::upperLine()
{
    return m_staves.empty() ? 16.0 : firstStaff()->upperLine();
}
...
Tnote TscoreObject::highestNote()
{
    ...
    case Tclef::PianoStaffClefs:
        return Tnote(1, 4);
    default:
        return Tnote(); // percussion clef (no clef) and unsupported dropped bass
    }
}
...
void TscoreObject::enableActions()
{
    if (!m_deleteNoteAct) {
        ...
        connect(m_deleteNoteAct, &Taction::triggered, this, [=] {
            if (!m_readOnly && !m_singleNote && m_allowAdding)
                deleteNote(m_activeNote);
        });
        ...
        connect(m_insertNoteAct, &Taction::triggered, this, [=] {
            if (!m_readOnly && !m_singleNote && m_allowAdding)
                insertNote(m_activeNote);
        });
        ...
        connect(m_clearScoreAct, &Taction::triggered, this, [=] {
            if (!m_readOnly)
                clearScore();
        });
        ...
        connect(m_editModeAct, &Taction::triggered, this, [=] {
            if (!m_readOnly && !m_singleNote)
        ...
        m_tieAct = new Taction(QGuiApplication::translate("ScoreToolbox", "tie", "To translate it properly, check please meaning of 'tie' in musical context."),
                               tipbg,
                               this);
...
void TscoreObject::addStaff(TstaffItem *st)
{
    ...
    if (m_staves.count() == 1) { // initialize first measure of first staff
        ...
        connect(st, &TstaffItem::destroyed, this, [=] {
            emit staffDestroying(st->number());
        });
    }
...
void TscoreObject::updateClefOffset()
{
    switch (m_clefType) {
    case Tclef::Treble_G_8down:
        m_clefOffset.set(3, 1);
        break;
    case Tclef::Bass_F:
        m_clefOffset.set(5, 0);
        break;
    case Tclef::Alto_C:
        m_clefOffset.set(4, 1);
        break;
    case Tclef::Tenor_C:
        m_clefOffset.set(2, 1);
        break;
    default:
        m_clefOffset.set(3, 2);
        break; // Treble, piano staff and no clef (rhythm only)
    }
}
