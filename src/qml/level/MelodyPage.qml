...
Tflickable {
    ...
    function checkMelodyView() {
        if (melCombo.currentIndex === 2) {
            if (!melListView) {
                ...
                melListView.width = Qt.binding(function() {
                    return melodyCol.width - 10;
                });
            }
    ...
    function updateNotesList() {
        ...
        for (var i = 0; i < creator.notesInList(); ++i) listScore.addNote(creator.noteFromList(i))
    }
    ...
    function saveLevel() {
        if (melCombo.currentIndex === 1) {
            ...
            for (var i = 0; i < listScore.notesCount; ++i) creator.setNoteOfList(i, listScore.scoreObj.noteAt(i))
    ...
    Column {
        ...
        Tile {
            ...
            property var descList: [qsTr("Melodies are composed from notes in range defined on the 'Range' page."), qsTr("Melodies are composed from notes selected on the score below."), qsTr("Melodies are selected from the list below.")]
        ...
        Item {
            width: parent.width //- tempoRange.height
            height: melPage.height - topTile.height - melGrid.height
        ...
        Tile {
            ...
            Row {
                ...
                Text {
                    anchors.verticalCenter: parent.verticalCenter
                    text: Math.floor(tempoRange.first.value)
                    color: enabled ? activPal.text : disdPal.text
                ...
                Text {
                    anchors.verticalCenter: parent.verticalCenter
                    text: Math.floor(tempoRange.second.value)
                    color: enabled ? activPal.text : disdPal.text
