...
TupdateItem {
  ...
  Column {
    ...
    GlowRect {
      ...
      Tflickable {
        anchors.fill: parent
        contentWidth: width; contentHeight: chText.height + NOO.factor()
        LinkText {
          id: chText
    ...
    Column {
      ...
      Tile {
        ...
        Row {
          ...
          ButtonGroup { buttons: periodCol.children }
          Column {
            id: periodCol
            spacing: NOO.factor() * (NOO.isAndroid() ? 0.5 : 1)
            TcheckBox {
          ...
          ButtonGroup { buttons: versionCol.children }
          Column {
            id: versionCol
  ...
  function accepted() { dialLoader.close() }
}
