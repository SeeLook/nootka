        ...
        if (err == Texam::e_file_OK || err == Texam::e_file_corrupted) {
            ...
            if (err == Texam::e_file_corrupted)
                ...
                QTimer::singleShot(50, this, [=] {
                    emit examSummary();
                }); // Allow 'Start Exam dialog' to be closed
            return;
        } else {
            message(tr("File: %1 \n is not valid exam file!").arg(QString("<b>%1</b>").arg(arg.toString())).replace(QLatin1String("\n"), QLatin1String("<br>")),
                    Qt::red,
                    AfterClose);
            return;
        }
...
void TexamExecutor::continueInit()
{
    ...
    if (m_questList.isEmpty() && !m_level.isMelodySet()) {
        message(tr("Level <b>%1</b><br>makes no sense because there are no questions to ask.<br>It can be re-adjusted.<br>Repair it in Level Creator and try "
                   "again.")
                    .arg(m_level.name),
                Qt::red,
                AfterClose);
...
void TexamExecutor::initializeExecuting()
{
    ...
    if (m_level.questionAs.isOnScore())
        m_level.answersAs[TQAtype::e_onScore].randNext();
    if (m_level.questionAs.isName())
        m_level.answersAs[TQAtype::e_asName].randNext();
    if (m_level.questionAs.isOnInstr())
        m_level.answersAs[TQAtype::e_onInstr].randNext();
    if (m_level.questionAs.isSound())
        m_level.answersAs[TQAtype::e_asSound].randNext();
    if (m_rand)
        m_rand->reset();
    else
...
void TexamExecutor::askQuestion(bool isAttempt)
{
    ...
    if (!curQ->answerAsSound() && SOUND->stoppedByUser())
        ...
        QTimer::singleShot(WAIT_TIME + 100, this, [=] {
            m_tipHandler->showQuestionTip();
        }); // So call it with delay
...
void TexamExecutor::checkAnswer(bool showResults)
{
    ...
    if (m_exercise) {
        if ((GLOB->examParams->autoNextQuest && GLOB->examParams->afterMistake != TexamParams::e_continue) || !GLOB->examParams->autoNextQuest
            || GLOB->examParams->showCorrected)
...
void TexamExecutor::correctAnswer()
{
    ...
    if (curQ->answerOnScore()) {
        ...
        if (!m_answRequire.accid && curQ->isNotSoBad()) { // respect accidental selected by user
            ...
            case -2:
                goodNote = goodNote.showWithDoubleFlat();
                break;
            case -1:
                goodNote = goodNote.showWithFlat();
                break;
            case 0:
                goodNote = goodNote.showAsNatural();
                break;
            case 1:
                goodNote = goodNote.showWithSharp();
                break;
            case 2:
                goodNote = goodNote.showWithDoubleSharp();
                break;
            }
        }
...
void TexamExecutor::createActions()
{
    ...
    if (m_level.questionAs.isSound()) {
        ...
        m_playAgainAct->setTip(
            tr("play sound again") + QStringLiteral("<br>(")
    ...
    if (m_exercise) {
        m_correctAct =
            new Taction(QApplication::translate("TtoolBar", "Correct", "like a correct answer with mistake"), QStringLiteral("correct"), this, false);
    ...
    if (m_exercise) {
        ...
        msg->setText(tr("Psssst... Exam is going.<br><br>"
                        ...
                        "or <b>%2</b> to continue.<br>")
                         .arg(qTR("QShortcut", "Save"), qTR("QPlatformTheme", "Retry")));
...
TtipHandler *TexamExecutor::tipHandler()
{
    return m_tipHandler;
}
