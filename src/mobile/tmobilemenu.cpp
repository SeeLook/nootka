    ...
    : QQuickItem(parent)
{
    ...
    m_pitchDetectAct =
        new Taction(QGuiApplication::translate("MainMenuMobile", "Pitch recognition", "Android menu entry, could be 'Note recognition' or 'detection' as well"),
                    QStringLiteral("mic"),
                    this);
...
void TmobileMenu::singleNoteModeSlot()
{
    if (GLOB->isSingleNote())
        ...
        setFlyActions(MAIN_SCORE->playAct(), m_tempoAct, MAIN_SCORE->scoreMenuAct(), m_clearFlyAct, m_pitchDetectAct);
}
