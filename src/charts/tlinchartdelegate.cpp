...
int TlinChartDelegate::groupNr() const
{
    return m_qInf->grNr();
}
...
QString TlinChartDelegate::nrText() const
{
    ...
    if (m_qInf->qaUnit() && m_qInf->qaUnit()->melody())
        att = QLatin1String("<br><font size=\"1\">(") + QGuiApplication::translate("Texam", "%n attempt(s)", "", m_qInf->qaUnit()->attemptsCount())
            + QLatin1String(")</font>");
...
{
    if (m_qInf->qaUnit() && m_qInf->qaUnit()->exam() && Tinstrument(m_qInf->qaUnit()->exam()->level()->instrument).isGuitar() && !m_qInf->qaUnit()->melody()) {
        if (m_qInf->qaUnit()->questionAs == TQAtype::e_onInstr || m_qInf->qaUnit()->answerAs == TQAtype::e_onInstr
            || m_qInf->qaUnit()->answerAs == TQAtype::e_asSound)
            return QLatin1String("<span style=\"font-family: 'Nootka';\">") + QString::number(static_cast<int>(m_qInf->qaUnit()->qa.pos().str()))
                + QLatin1String("</span>") + TfingerPos::romanFret(m_qInf->qaUnit()->qa.pos().fret());
    }
    return QString();
}
...
{
    m_qInf->setCursorPos(
        QPointF(parentItem()->x(), pointY())); // delegate is wrapped by QML Loader, so just this item has proper x coordinate towards view area
...
void TlinChartDelegate::lineEntered(qreal posX, qreal posY)
{
    ...
    if (m_chart->settings()->order == Tchart::e_byNumber)
        m_chart->lineTip()->tipText =
            TexTrans::averAnsverTimeTxt() + QString("<br><big><b>%1</b></big>").arg(Texam::formatReactTime(m_chart->exam()->averageReactonTime(), true));
    else {
        auto gr = m_chart->mainChart()->group(m_qInf->grNr());
        m_chart->lineTip()->tipText =
            gr->fullDescription() + QLatin1String("<br><big><b>") + Texam::formatReactTime(qRound(gr->averTime()), true) + QLatin1String("</b></big>");
    }
    ...
    if (m_chart->settings()->order != Tchart::e_byNumber) {
        ...
        if (nrInGr > -1 && gr->size() / 2 == nrInGr) {
            ...
            if (m_chart->settings()->order == Tchart::e_byFret)
                return QString("<span style=\"font-size: %1px;\">%2</span>")
                    .arg(qApp->font().pointSize() * 4)
                    .arg(TfingerPos::romanFret(gr->first()->qa.pos().fret()));
...
void TlinChartDelegate::setProgressHoverred(bool ph)
{
    if (ph != m_progressHoverred) {
        ...
        if (m_progressHoverred) {
            ...
            m_chart->lineTip()->tipText =
                QLatin1String("<b>") + QGuiApplication::translate("TgraphicsLine", "progress line").toUpper()
                + QLatin1String(
                    "</b><br><span style=\"color: #ff00ff; font-size: x-large; font-family: 'Courier New', Courier, monospace;\"><b> \\ </b></span>")
...
{
    if (m_chart->settings()->order == Tchart::e_byNumber && (!m_chart->isMelody() || m_chart->settings()->yValue == Tchart::e_YquestionTime) && m_nrInchart > 0
        && m_chart->mainChart()->averChunk(m_nrInchart - 1) > 0.0) {
        qreal h = height() * HH;
